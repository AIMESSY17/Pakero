...
export function MinijuegoOverlay({ tipo, bonusCombate = 0, contexto, onFin }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-noche/92 p-4 backdrop-blur-sm">
      <div className="anim-subir w-full max-w-4xl">
        {jugando ? (
          <PhaserGame tipo={tipo} bonusCombate={bonusCombate} onResultado={(score) => onFin(score)} />
        ) : (
          <div className="rounded-2xl border border-borde bg-panel p-7 text-center shadow-2xl">
            <div aria-hidden className="text-5xl">
              {def.icono}
            </div>
            <h2 className="mt-3 font-display text-3xl text-tiza">{def.config.titulo}</h2>
            ...
            <div className="mx-auto mt-5 max-w-md rounded-xl border border-borde bg-panel-alto p-3 text-xs text-humo">
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <Boton onClick={() => setJugando(true)}>Jugarlo</Boton>
              <Boton variante="fantasma" onClick={() => onFin(null)}>
                Saltear (sin bonus)
              </Boton>
            </div>
          </div>
